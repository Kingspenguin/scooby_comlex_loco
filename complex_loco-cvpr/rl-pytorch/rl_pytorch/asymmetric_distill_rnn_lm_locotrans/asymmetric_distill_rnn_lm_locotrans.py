...
from .storage import LMRolloutStorage, hidden_state_to_one_tensor
...
class AsymmetricDistillRNNLMLocoTransTrainer:
  def __init__(
      self,
      vec_env,
    ...
    self.storage = LMRolloutStorage(
        ...
        self.action_space.shape, self.hidden_state_size, self.hidden_state_num,
    ...
    if self.is_testing:
      ...
      for it in range(self.current_learning_iteration, num_learning_iterations):
        ...
        for _ in range(self.num_transitions_per_env):
          ...
          # Step the vec_environment
          # if self.alterante_sampling and it % 2 == 0:
  ...
  def log(self, locs, width=80, pad=35):
    ...
    # self.writer.add_scalar(
    #     'Loss/bc_loss', locs['mean_bc_loss'], locs['it'])

    # self.writer.add_scalar(
    #     'Loss/lm_loss', locs['mean_lm_loss'], locs['it'])
...
def one_tensor_to_hidden_state(hidden_state_tensor: Tensor, hidden_state_size: int, hidden_state_num: int) -> List[Tuple[Tensor, Tensor]]:
  ...
  for i in range(hidden_state_num):
    # print(hidden_state_tensor[..., 2 * i *
    #       hidden_state_size:(2 * i + 1) * hidden_state_size].dtype)
