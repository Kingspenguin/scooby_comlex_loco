...
class LocotransRNNCPGPolicyWrapper():
  def __init__(
      self,
      policy,
      obs_scale,
    ...
    self.nominal_residual_phase = 1.0 * \
        np.array([-phase_scale, phase_scale, phase_scale, -phase_scale])
  ...
  def get_action(
          self, observation, hidden_state, depth_frame, depth_scale, last_action):
    '''
    ...
    if self.save_log:
      self.policy_action_saver.record(action.detach().cpu().numpy())
