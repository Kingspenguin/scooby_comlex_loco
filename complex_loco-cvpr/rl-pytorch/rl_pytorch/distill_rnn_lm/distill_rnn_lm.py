...
class DistillRNNLMTrainer:
  def __init__(
    ...
    self.use_l1 = True
    self.use_exp_imi = exp_imi_coeff is not None
    self.exp_imi_coeff = exp_imi_coeff
        ...
        for _ in range(self.num_transitions_per_env):
          ...
          if self.eval_env_nums > 0:
            with torch.no_grad():
              ...
              # print(current_obs[-self.eval_env_nums:].shape)
              # print(current_states[-self.eval_env_nums:].shape)
              # print(mu[-self.eval_env_nums:].shape)
