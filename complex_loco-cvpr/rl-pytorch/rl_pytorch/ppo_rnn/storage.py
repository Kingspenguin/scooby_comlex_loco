...
class RolloutStorage:
  ...
  def __init__(self, num_envs, num_transitions_per_env, obs_shape, states_shape, 
    actions_shape, hidden_state_size, hidden_state_num, device='cpu', sampler='sequential'
  ):
    ...
    self.dones = torch.zeros(num_transitions_per_env,
                             num_envs, 1, device=self.device).byte()
    ...
    self.hidden_states = torch.zeros(
        hidden_state_num, num_transitions_per_env,
        num_envs, hidden_state_size, device=self.device)
  ...
  def add_transitions(
    ...
    observations, states,
    actions, rewards,
    dones, values, actions_log_prob, mu, sigma, hidden_states
  ):
