...
class AsymmetricDistillLMLocoTransTrainer:
  def __init__(
      self,
      vec_env,
    ...
    self.alternate_sampling = alternate_sampling
    ...
    self.storage = LMRolloutStorage(
        ...
        self.device, sampler,
        with_teacher_mask=self.alternate_sampling
    )
  ...
  def student_load(self, path, device='cuda:0'):
    ...
    if self.alternate_sampling:
      self.student_rewbuffer = check_point['student_rewbuffer']
      self.student_lenbuffer = check_point['student_lenbuffer']

      self.teacher_rewbuffer = check_point['teacher_rewbuffer']
      self.teacher_lenbuffer = check_point['teacher_lenbuffer']
    else:
  ...
  def save(self, path):
    ...
    if self.alternate_sampling:
      checkpoint = {
          ...
          'student_rewbuffer': self.student_rewbuffer,
          'student_lenbuffer': self.student_lenbuffer,
          'teacher_rewbuffer': self.teacher_rewbuffer,
          'teacher_lenbuffer': self.teacher_lenbuffer
      }
    else:
      checkpoint = {
          'student_ac': self.student_actor_critic.state_dict(),
  ...
  def run(self, num_learning_iterations, log_interval=1):
    ...
    if self.is_testing:
      ...
      if self.alternate_sampling:
        student_cur_reward_sum = torch.zeros(
            self.vec_env.num_envs // 2, dtype=torch.float, device=self.device)
        student_cur_episode_length = torch.zeros(
            self.vec_env.num_envs // 2, dtype=torch.float, device=self.device)

        teacher_cur_reward_sum = torch.zeros(
            self.vec_env.num_envs // 2, dtype=torch.float, device=self.device)
        teacher_cur_episode_length = torch.zeros(
            self.vec_env.num_envs // 2, dtype=torch.float, device=self.device)
      else:
      ...
      for it in range(self.current_learning_iteration, num_learning_iterations):
        ...
        if self.alternate_sampling:
          student_reward_sum = []
          student_episode_length = []

          teacher_reward_sum = []
          teacher_episode_length = []
        else:
          reward_sum = []
          episode_length = []
        ...
        for _ in range(self.num_transitions_per_env):
          ...
          # Step the vec_environment
          # if self.alternate_sampling and it % 2 == 0:
          #   next_obs, rews, dones, infos = self.vec_env.step(teacher_acts)
          # else:
          prev_dones = dones.clone()
          if self.alternate_sampling:
            actions_for_env = torch.cat([
                student_actions[:self.vec_env.num_envs // 2],
                teacher_acts[self.vec_env.num_envs // 2:]
            ], dim=0)
            # actions_for_env = teacher_acts
          else:
            actions_for_env = student_actions
          ...
          if self.alternate_sampling:
            teacher_masks = torch.zeros_like(rews, device=rews.device)
            teacher_masks[self.vec_env.num_envs // 2:] = 1
            self.storage.add_transitions(
                # current_obs,
                non_privilege_obs, privilege_obs,
                student_actions, rews, dones, prev_dones, values, student_actions_log_prob, mu, sigma,
                teacher_acts, teacher_feature, teacher_masks=teacher_masks
            )
          else:
            self.storage.add_transitions(
                # current_obs,
          ...
          if self.print_log:
            if self.alternate_sampling:
              student_cur_reward_sum[:] += rews[:self.vec_env.num_envs // 2]
              student_cur_episode_length[:] += 1

              teacher_cur_reward_sum[:] += rews[self.vec_env.num_envs // 2:]
              teacher_cur_episode_length[:] += 1
            else:
              cur_reward_sum[:] += rews
            ...
            if self.alternate_sampling:
              ...
              student_reward_sum.extend(
                  student_cur_reward_sum[student_new_ids][:, 0].cpu().numpy().tolist())
              student_episode_length.extend(
                  student_cur_episode_length[student_new_ids][:, 0].cpu().numpy().tolist())
              student_cur_reward_sum[student_new_ids] = 0
              student_cur_episode_length[student_new_ids] = 0

              teacher_new_ids = (
                  fake_dones[self.vec_env.num_envs // 2:] > 0
              ).nonzero(as_tuple=False)
              teacher_reward_sum.extend(
                  teacher_cur_reward_sum[teacher_new_ids][:, 0].cpu().numpy().tolist())
              teacher_episode_length.extend(
                  teacher_cur_episode_length[teacher_new_ids][:, 0].cpu().numpy().tolist())
              teacher_cur_reward_sum[teacher_new_ids] = 0
              teacher_cur_episode_length[teacher_new_ids] = 0

            else:
              new_ids = (fake_dones > 0).nonzero(as_tuple=False)
              reward_sum.extend(
        ...
        if self.print_log:
          ...
          if self.alternate_sampling:
            self.student_rewbuffer.extend(student_reward_sum)
            self.student_lenbuffer.extend(student_episode_length)

            self.teacher_rewbuffer.extend(teacher_reward_sum)
            self.teacher_lenbuffer.extend(teacher_episode_length)
          else:
        ...
        if self.print_log:
          if self.alternate_sampling:
            student_rewbuffer = copy.deepcopy(self.student_rewbuffer)
            student_lenbuffer = copy.deepcopy(self.student_lenbuffer)

            teacher_rewbuffer = copy.deepcopy(self.teacher_rewbuffer)
            teacher_lenbuffer = copy.deepcopy(self.teacher_lenbuffer)
          else:
    ...
    if self.alternate_sampling:
      if len(locs['student_rewbuffer']) > 0:
        log_string = (f"""{'#' * width}\n"""
                      ...
                      f"""{'Student Mean reward:':>{pad}} {statistics.mean(locs['student_rewbuffer']):.2f}\n"""
                      # f"""{'Teacher Mean reward:':>{pad}} {statistics.mean(locs['teacher_rewbuffer']):.2f}\n"""
                      f"""{'Student Mean episode length:':>{pad}} {statistics.mean(locs['student_lenbuffer']):.2f}\n"""
                      f"""{'Mean reward/step:':>{pad}} {locs['mean_reward']:.2f}\n"""
                      f"""{'Mean episode length/episode:':>{pad}} {locs['mean_trajectory_length']:.2f}\n""")
        if len(locs['teacher_rewbuffer']) > 0:
          log_string += (
              f"""{'Teacher Mean reward:':>{pad}} {statistics.mean(locs['teacher_rewbuffer']):.2f}\n"""
              f"""{'Teacher Mean episode length:':>{pad}} {statistics.mean(locs['teacher_lenbuffer']):.2f}\n"""
          )
        if not self.local:
          log_dict = {
              ...
              'Student Mean reward:': statistics.mean(locs['student_rewbuffer']),
              'Student Mean episode length:': statistics.mean(locs['student_lenbuffer']),
              'Mean reward/step:': locs['mean_reward'],
              'Mean episode length/episode:': locs['mean_trajectory_length']
          }
          if len(locs['teacher_rewbuffer']) > 0:
            log_dict["Teacher Mean reward:"] = statistics.mean(
                locs['teacher_rewbuffer'])
            log_dict["Teacher Mean episode length:"] = statistics.mean(
                locs['teacher_lenbuffer'])
          wandb.log(log_dict)
      else:
        log_string = (f"""{'#' * width}\n"""
                      f"""{str.center(width, ' ')}\n\n"""
  ...
  def update(self):
    ...
    for epoch in range(self.num_learning_epochs):
      for indices in batch:
        ...
        if self.alternate_sampling:
          teacher_masks_batch = self.storage.teacher_masks[:, indices].detach()
        ...
        if self.alternate_sampling:
          surrogate_loss = (
              ...
              torch.squeeze(1 - teacher_masks_batch)
          ).mean()
        else:
