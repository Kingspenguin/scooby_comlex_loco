...
from a1_utilities.a1_sensor_process import convert_order_from_isaac_to_ros, convert_order_from_ros_to_isaac, observation_to_joint_pos_and_vel_isaac, observation_to_joint_vel, observation_to_torque
...
class CPGPolicyWrapper():
  def __init__(
    ...
    # FIXME: need to see what is this nominal residual phase
    self.nominal_residual_phase = 1.0 * np.array([-0.1, 0.1, 0.1, -0.1])
  ...
  def process_obs(
    ...
    if not self.vis_only:
      ...
      base_ang_vel_hist_normalized = np.array(
      ...
      # joint angle
      # joint_angle, joint_vel = observation_to_joint_pos_and_vel_isaac(observation)
      ...
      # if self.use_com_vel:
      com_vel_normalized = com_vel * self.obs_scale["com_vel"]

      # if self.use_command:
      command_normalized = np.zeros((3,))
    ...
    if self.vis_only:
      ...
      #   print("Phase Info", phase_info_hist_normalized)  # 8)
    else:
      obs_list = []
      # if self.use_com_vel:
  ...
  def process_act(self, action):
    if self.vis_only:
      ...
      # print(self.action_scale, action_normalized)
  ...
  def get_action(
    ...
    print("process time", infer_start - process_start)
    with torch.no_grad():
      action = self.pf.act_inference(ob_t)
    infer_end = time.time()
    print("infer time", infer_end - infer_start)
    ...
    if self.save_log:
      self.policy_action_saver.record(action.cpu().numpy())
    print("CPG Time:", time.time() - infer_end)
    # print("Processed Act:", action)
    # print("Target Pos:", target_joint_pos)
    return action, target_joint_pos
  ...
  def get_cpg_action(self, action):
    ...
    # dof_pos = self.current_joint_angle
    # dof_vel = self.current_joint_vel
    # # print("----------------------")
    # # print("Current Joint Pos:", dof_pos)
    # # print("Current Joint Vel:", self.current_joint_vel)
    # # print("Raw Target Joint Pos:", target_joint_pos)

    # torque = (target_joint_pos - dof_pos) * self.Kp - self.Kd * dof_vel
    # # print("Raw Torque:", torque)
    # clipped_toruqe = np.sign(torque) * np.clip(
    #     np.abs(torque), 0, 33.0
    # )
    # clipped_target_joint_pos = (
    #     clipped_toruqe + self.Kd * dof_vel
    # ) / self.Kp + dof_pos
    # # print("Clipped Target Joint Pos:", clipped_target_joint_pos)
    # # print("Raw Act:", action)
    # # print("Clipped Torque:", (clipped_action - task.dof_pos) * 60)
    # # print("Clipped Act:", clipped_action)
    # # task.gym.set_dof_position_target_tensor(
    # #     task.sim, gymtorch.unwrap_tensor(clipped_action)
    # target_joint_pos = convert_order_from_isaac_to_ros(
    #     clipped_target_joint_pos)
    # target_torque = convert_order_from_isaac_to_ros(clipped_toruqe)
    target_joint_pos = convert_order_from_isaac_to_ros(target_joint_pos)
