...
class NormedStateHistory():
  ...
  def record(self, sensor_input, backwards=False):
    '''
    ...
    if backwards is True:
      self.sensor_history = np.vstack(
          (self.sensor_history[1:, :], sensor_input))
    else:
      self.sensor_history = np.vstack(
          (sensor_input, self.sensor_history[:-1, :]))
    # normalize
    return self.sensor_history.reshape(-1)
  ...
  def record_and_normalize(self, sensor_input, mean=None, std=None, backwards=False):
    ...
    # print(self.sensor_history.reshape(-1), mean, std)
    # normalize
    return (self.sensor_history.reshape(-1) - mean) / (std + 1e-4)


def depth_process(depth, use_inverse_depth, use_original_depth):
  ...
  def record_and_normalize(self, frame, depth_scale, backwards=False):
    ...
    if self.sliding_frames:
      ...
      unrolled_depth_history = np.array((
          ...
          unrolled_depth_history[64 * 64 * 12:64 * 64 * 13],
          unrolled_depth_history[64 * 64 * 16:64 * 64 * 17],
          # unrolled_depth_history[64 * 64 * 15:]
      )
      ).flatten()
