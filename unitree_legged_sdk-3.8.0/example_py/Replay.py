...
def move_to_pose_init(udp,safe,cmd,state,
  target_joint_position,
  interpolate_duration=0.1,
  after_duration=0.02):


  cmd_joint_pos=target_joint_position
  freq = 400
  while True:
        time.sleep(0.002)
        udp.Recv()
  ...
  for i in range(int((interpolate_duration + after_duration) * freq)):
        ...
        for j in range (len(target_joint_position)):
                cmd.motorCmd[j].q = np.float32(cmd_joint_pos[j])
                #cmd.motorCmd[j].dq = 1
                cmd.motorCmd[j].Kp = 60
                cmd.motorCmd[j].Kd = 4.0
...
def move_to_pose(udp,safe,cmd,state,
  target_joint_position,
  interpolate_duration=0.1,
  after_duration=0.02):
...
#         if (check_joint_angle_sanity(init_joint_angles)):
#             break
#   state_angles = np.zeros(12, dtype=np.float32)
...
#   init_joint_angles = np.zeros(12, dtype=np.float32)
  cmd_joint_pos=target_joint_position
#   for x in range(len(init_joint_angles)):
#                     init_joint_angles[x]=state.motorState[x].q
        # cmd_joint_pos = interpolate_joint_position(
        #     init_joint_angles,
        #     target_joint_position,
        #     i / int(interpolate_duration * freq))
  for j in range (len(target_joint_position)):
        ...
        torque = (0 - state.motorState[j].q)*40.0 + (0 - state.motorState[j].dq)*4.0
        torque = np.fmin(np.fmax(torque, -5.0), 5.0)
        cmd.motorCmd[j].q = cmd_joint_pos[j]
                # state_angles[j]=state.motorState[j].q
        cmd.motorCmd[j].dq = 0
        cmd.motorCmd[j].Kp = 60
        cmd.motorCmd[j].Kd = 3.0
        cmd.motorCmd[j].tau =0
        # print(cmd.motorCmd[0].q)
        #print("state:",i,state_angles)
...
#   udp.Send()
  return cmd
def move_to_stand(udp,safe,cmd,state):
    motiontime = 0
...
,
    ...
    for motiontime in range(int((interpolate_duration + stand_duration) * freq)):
        t1 = time.time()
        udp.Recv()
        udp.GetRecv(state)
...
def move_to_sit(udp,safe,cmd,state):
    motiontime = 0
...
def check_joint_angle_sanity(joint_position):
    ...
    A1_joint_angle_limits = np.array([
        [-1.047,    -0.663,      -2.9],  # Hip, Thigh, Calf Min
...
,
    ...
    udp.Send()
    cmd=move_to_stand(udp,safe,cmd,state)
    traj = np.load("joint_target_pos_recording_2.pkl",allow_pickle=True)
    cmd=move_to_pose_init(udp,safe,cmd,state, traj[200], 1.0, 1.0)
    # print(type(traj[0][0]))
    for pose in traj[200:]:
        t1 = time.time()
        print(pose)
        
        # if i==0:
        #     cmd=move_to_pose_init(udp,safe,cmd,state, traj[0], 1.0, 2.0)
        # else:
        #     cmd=move_to_pose(udp,safe,cmd,state, traj[i], 0.015, 0.005)
        #print("target:",pose)
        cmd=move_to_pose(udp,safe,cmd,state,pose, 0.003, 0.002)
        t2 = time.time()
        time.sleep(max(0, 1 / 200 - (t2 - t1)))
        
    # cmd=move_to_pose(udp,safe,cmd, traj[0], 0.045, 0.05)
    cmd=move_to_sit(udp,safe,cmd,state)
    # while True:
    #     udp.SetSend(cmd)
    #     udp.Send()
    #     time.sleep(0.02)
