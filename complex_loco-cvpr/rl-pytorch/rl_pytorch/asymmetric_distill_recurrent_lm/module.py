...
class RecurrentActorCriticLM(nn.Module):
  ...
  def act(self, observations, states, hidden_states, masks):
  ...
  def act_inference(self, observations, hidden_states, masks):
    rnn_out, new_hidden_states = self.encoder(
        observations, hidden_states, masks)
    if rnn_out.shape[0] == 1:
      rnn_out = rnn_out.squeeze(0)
  ...
  def evaluate(self, observations, states, actions, hidden_states, masks, return_rnnout=False):
    rnn_out, new_hidden_states = self.encoder(
        observations, hidden_states, masks)
    if rnn_out.shape[0] == 1:
      rnn_out = rnn_out.squeeze(0)
    actions_mean = self.actor(rnn_out)
    if return_rnnout:
      rnn_out = self.actor[0](rnn_out)
      rnn_out = self.actor[1](rnn_out)
      rnn_out = self.actor[2](rnn_out)
      rnn_out = self.actor[3](rnn_out)
    ...
    if return_rnnout:
      return actions_log_prob, entropy.unsqueeze(-1), value, actions_mean, self.log_std.unsqueeze(0).unsqueeze(1).repeat(actions_mean.shape[0], actions_mean.shape[1], 1), new_hidden_states, rnn_out
