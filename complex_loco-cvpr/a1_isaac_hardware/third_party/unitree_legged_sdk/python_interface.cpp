...
public:
    ...
    ProcessedLowState ReceiveProcessedObservation();
    HighState ReceiveHighState();
    void SendCommand(std::array<float, 60> motorcmd);
    void SendKPCommand(std::array<float, 12> motorcmd, float Kp, float Kd);
    void SendHighCommand(std::array<float, 3> raw_high_cmd);
    void Initialize();

    UDP udp;
    Safety safe;
    LowState state = {0};
    ProcessedLowState p_state = {0};
    LowCmd cmd = {0};
    HighCmd high_cmd = {0};
...
ProcessedLowState RobotInterface::ReceiveProcessedObservation() {
    udp.Recv();
    udp.GetRecv(state);
    for (int motor_id = 0; motor_id < 12; motor_id++){
        p_state.q[motor_id] = state.motorState[motor_id].q;
        p_state.dq[motor_id] = state.motorState[motor_id].dq;
    }
    p_state.imu = state.imu;
    return p_state;
}

void RobotInterface::SendKPCommand(std::array<float, 12> motorcmd, float Kp, float Kd) {
    cmd.levelFlag = 0xff;
    for (int motor_id = 0; motor_id < 12; motor_id++) {
...
HighState RobotInterface::ReceiveHighState() {
    udp.Recv();
    udp.GetRecv(high_state);
    return high_state;
}

void RobotInterface::SendHighCommand(std::array<float, 3> raw_high_cmd) {
    high_cmd.levelFlag = 0x00;
    high_cmd.mode = raw_high_cmd[0];
    high_cmd.forwardSpeed = raw_high_cmd[1];
    high_cmd.rotateSpeed = raw_high_cmd[2];
...
PYBIND11_MODULE(robot_interface, m) {
    ...
    py::class_<ProcessedLowState>(m, "ProcessedLowState")
        .def(py::init<>())
        .def_readwrite("q", &ProcessedLowState::q)
        .def_readwrite("dq", &ProcessedLowState::dq)
        .def_readwrite("imu", &ProcessedLowState::imu);

    py::class_<RobotInterface>(m, "RobotInterface")
        .def(py::init<>())
        .def(py::init<int>())
        .def("receive_observation", &RobotInterface::ReceiveObservation)
        .def("receive_processed_observation", &RobotInterface::ReceiveProcessedObservation)
        .def("receive_high_state", &RobotInterface::ReceiveHighState)
        .def("send_command", &RobotInterface::SendCommand)
        .def("send_kp_command", &RobotInterface::SendKPCommand)
