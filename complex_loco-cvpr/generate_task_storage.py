# Copyright (c) 2020, NVIDIA CORPORATION.  All rights reserved.
# NVIDIA CORPORATION and its licensors retain all intellectual property
# and proprietary rights in and to this software, related documentation
# and any modifications thereto.  Any use, reproduction, disclosure or
# distribution of this software and related documentation without an express
# license agreement from NVIDIA CORPORATION is strictly prohibited.

import numpy as np
import random
import os
import sys
import os.path as osp
sys.path.append(os.path.join(os.path.dirname(__file__), ".."))
print(os.path.join(os.path.dirname(__file__), ".."))

from utils.config import set_np_formatting, set_seed, get_args, parse_sim_params, load_cfg
from utils.parse_task import parse_task
from utils.process_storage_generator import process_storage_generator

import torch

import wandb


def train():
  seed = cfg_train.get("seed", -1)
  d = {}
  d.update(cfg)
  d.update(cfg_train)
  if not args.test:
    wandb.init(
      project="MultiViewLocomotion",
      name="{}_{}_{}{}_{}".format(
        cfg["env"]["name"],
        cfg["task"]["name"],
        cfg_train["learn"]["agent_name"],
        args.add_label,
        str(seed)
      ),
      group="{}_{}_{}{}".format(
        cfg["env"]["name"],
        cfg["task"]["name"],
        cfg_train["learn"]["agent_name"],
        args.add_label
      ),
      # job_type=env_name,
      config=d
    )

  task, env = parse_task(args, cfg, cfg_train, sim_params)
  storage_generator = process_storage_generator(
    args, env, cfg_train, teacher_logdir
  )

  dapg_iterations = cfg_train["learn"]["max_iterations"]
  if args.max_iterations > 0:
    dapg_iterations = args.max_iterations

  storage_generator.run()


if __name__ == '__main__':
  set_np_formatting()
  args = get_args()
  cfg, cfg_train, student_logdir = load_cfg(args)
  teacher_logdir = args.teacher_logdir
  sim_params = parse_sim_params(args, cfg, cfg_train)
  set_seed(cfg_train.get("seed", -1),
           cfg_train.get("torch_deterministic", False))
  train()
