...
class ActorCriticLM(nn.Module):
  ...
  def evaluate(self, observations, states, actions, return_rnnout=False):
    rnn_out = self.encoder(observations)
    if rnn_out.shape[0] == 1:
      rnn_out = rnn_out.squeeze(0)
    actions_mean = self.actor(rnn_out)
    if return_rnnout:
    ...
    if return_rnnout:
      return actions_log_prob, entropy.unsqueeze(-1), value, actions_mean, self.log_std.unsqueeze(0).unsqueeze(1).repeat(actions_mean.shape[0], actions_mean.shape[1], 1), rnn_out
