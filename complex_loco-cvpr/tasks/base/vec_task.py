...
class VecTaskCPU(VecTask):
  def __init__(self, task, rl_device, sync_frame_time=False, clip_observations=5.0, clip_actions=1.0):
    super().__init__(task, rl_device,
                     clip_observations=clip_observations, clip_actions=clip_actions)
  ...
  def reset(self):
    actions = 0.01 * (1 - 2 * np.random.rand(self.num_envs,
                      self.num_actions)).astype('f')
...
class VecTaskGPU(VecTask):
  ...
  def reset(self):
    actions = 0.01 * (1 - 2 * torch.rand([self.task.num_envs,
                      self.task.num_actions], dtype=torch.float32, device=self.rl_device))
...
class VecTaskPython(VecTask):
  ...
  def step(self, actions):
    ...
    self.task.step(actions_tensor)
    self.task.extras["fake_done"] = self.task.extras["fake_done"].to(
        self.rl_device
    )
