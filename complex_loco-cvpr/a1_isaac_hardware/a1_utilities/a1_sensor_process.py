...
# The following two function has exactlly the same function
# But with different semantic meaning
def convert_order_from_ros_to_isaac(
    ros_joint_info
):
    # Order in Pybullet & Real Robot:
    #    FR, FL, RR, RL

    # Order in Isaac:
    #    FL, FR, RL, RR
    FR, FL, RR, RL = np.split(ros_joint_info, 4)
    isaac_joint_info = np.concatenate([
        FL, FR, RL, RR
    ])
    return isaac_joint_info


def convert_order_from_isaac_to_ros(
    isaac_joint_info
):
    # Order in Pybullet & Real Robot:
    #    FR, FL, RR, RL
    ...
    # Order in Isaac:
    #    FL, FR, RL, RR
    FL, FR, RL, RR = np.split(isaac_joint_info, 4)
    ros_joint_info = np.concatenate([
        FR, FL, RR, RL
    ])
    return ros_joint_info


def prepare_torque_cmd(target_torque):
    '''
    ...
    '''
    cmd = np.zeros((12, 5), dtype=np.float32)
    cmd[:, 0] = 2.14e9
    cmd[:, 2] = 16000.0
    cmd[:, 4] = target_torque
    cmd = cmd.reshape(-1)
...
def prepare_position_cmd_with_default(target_joint_position, default_cmd):
    '''
        Prepare low level command according to joint target position 
...
def observation_to_joint_pos_and_vel_isaac(observation):
    '''
    ...
    for i in range(12):
        ...
        joint_vel[i] = observation.motorState[i].dq

    return convert_order_from_ros_to_isaac(joint_position), convert_order_from_ros_to_isaac(joint_vel)

def observation_to_joint_position_isaac(observation):
    '''
    Extract joint position information from observation
    ...
    return convert_order_from_ros_to_isaac(joint_position)


def observation_to_joint_vel(observation):
    '''
    Extract joint position information from observation
    ...
    return convert_order_from_ros_to_isaac(joint_vel)
...
def observation_to_torque(observation):
    ...
    return convert_order_from_ros_to_isaac(torque)
...
def observation_to_joint_state(observation):
    ...
    return convert_order_from_ros_to_isaac(joint_state)
