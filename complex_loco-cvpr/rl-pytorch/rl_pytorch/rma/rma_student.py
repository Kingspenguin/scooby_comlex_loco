...
from .student_storage import RolloutStorage
...
class RMAStudent:
  def __init__(
    ...
    self.hidden_loss_coef = hidden_loss_coef
    self.gamma = gamma
    self.lam = lam
  ...
  def split_obs(self, obs_batch):
    ...
    historical_buffer = obs_batch[:, self.vec_env.task.state_obs_size +
                                  self.vec_env.task.image_obs_size + self.vec_env.task.height_map_obs_size:]
    ...
    env_factor = torch.cat([privilege_info, height], dim=1)
    inner_state_size = self.vec_env.task.state_obs_size - \
      self.vec_env.task.privilege_info_len
    historical_buffer = historical_buffer.view(historical_buffer.shape[0], -1, inner_state_size)
    ...
    return observations, states, env_factor, historical_buffer, inner_state_size
    ...
    if self.is_testing:
      ...
      while (not self.log_video or (step < self.vid_log_step)) and not self.vec_env.task.task_wrapper.test_finished:
        with torch.no_grad():
          if self.apply_reset:
            current_obs = self.vec_env.reset()
          # Compute the action
          observations, _, _, historical_buffer, inner_state_size = self.split_obs(
            current_obs)
          actions = self.student_actor_critic.act_inference(
            observations, env_factor=None, historical_buffer=historical_buffer, inner_state_size=inner_state_size)
      ...
      for it in range(self.current_learning_iteration, num_learning_iterations):
        ...
        for _ in range(self.num_transitions_per_env):
          ...
          observations, states, env_factor, historical_buffer, inner_state_size = self.split_obs(
            current_obs)
          # Compute the action
          with torch.no_grad():
            student_actions, student_actions_log_prob, values, mu, sigma = self.student_actor_critic.act(
              observations, states, env_factor=None, historical_buffer=historical_buffer, inner_state_size=inner_state_size)
        ...
        mean_value_loss, mean_surrogate_loss, mean_hidden_loss = self.update()
        self.storage.clear()
        stop = time.time()
  ...
  def log(self, locs, width=80, pad=35):
    ...
    self.writer.add_scalar(
      'Loss/surrogate', locs['mean_hidden_loss'], locs['it'])
    ...
    if len(locs['rewbuffer']) > 0:
      log_string = (f"""{'#' * width}\n"""
                    ...
                    f"""{'Hidden state loss:':>{pad}} {locs['mean_hidden_loss']:.4f}\n"""
      ...
      if not self.local:
        wandb.log({
          ...
          'Hidden state loss:': locs['mean_hidden_loss'],
  ...
  def update(self):
    ...
    for epoch in range(self.num_learning_epochs):
      ...
      for indices in batch:
        ...
        observations_batch, states_batch, env_factor_batch, historical_buffer_batch, inner_state_size = self.split_obs(
          obs_batch)
        actions_log_prob_batch, entropy_batch, value_batch, mu_batch, sigma_batch = self.student_actor_critic.evaluate(observations_batch,
                                                                                                                       states_batch,
                                                                                                                       actions_batch, env_factor=None, historical_buffer=historical_buffer_batch, inner_state_size=inner_state_size)
        # KL
        ...
        with torch.no_grad():
          teacher_hidden_vector_batch = self.teacher_actor_critic.get_hidden_vector(observations_batch,
            env_factor=env_factor_batch, inner_state_size=inner_state_size)
        student_hidden_vector_batch = self.student_actor_critic.get_hidden_vector(observations_batch,
          historical_buffer=historical_buffer_batch, inner_state_size=inner_state_size)
        hidden_loss = (teacher_hidden_vector_batch -
                       student_hidden_vector_batch).pow(2).mean()

        loss = surrogate_loss + self.value_loss_coef * \
          value_loss - self.entropy_coef * \
          entropy_batch.mean() + self.hidden_loss_coef * hidden_loss
        ...
        mean_hidden_loss += hidden_loss.item()

    num_updates = self.num_learning_epochs * self.num_mini_batches
    mean_value_loss /= num_updates
    mean_surrogate_loss /= num_updates
    mean_hidden_loss /= num_updates

    return mean_value_loss, mean_surrogate_loss, mean_hidden_loss
