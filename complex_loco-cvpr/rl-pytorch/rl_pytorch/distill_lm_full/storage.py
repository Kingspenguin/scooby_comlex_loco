...
class LMRolloutStorage:
  ...
  def __init__(
      ...
      actions_shape, teacher_feature_shape, device='cpu', sampler='sequential', with_teacher_mask=False
  ):
    ...
    self.values = torch.zeros(num_transitions_per_env,
                              num_envs, 1, device=self.device)
    self.obs_values = torch.zeros(num_transitions_per_env,
                                  num_envs, 1, device=self.device)
  ...
  def add_transitions(
      ...
      actions, rewards,
      dones, prev_dones, values, obs_values, actions_log_prob,
      mu, sigma,
    ...
    self.obs_values[self.step].copy_(obs_values)
