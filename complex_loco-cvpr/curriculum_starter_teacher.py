...
def run_exp():
  start_command = "{} {} --headless --num_envs {} --cfg_train {} --cfg_env {} --logdir {} --max_iterations {} --add_label {} --seed {}"
  base_command = "{} {} --headless  --num_envs {} --cfg_train {} --cfg_env {} --logdir {} --resume {} --max_iterations {} --add_label {} --seed {} --wandb_resume True"
  ...
  for idx, start_iter in enumerate(range(starting_iter, args.max_iter, args.interval)):
    if start_iter == 0:
      command = start_command.format(
          ...
          args.file,
          args.num_envs,
          args.cfg_train, args.cfg_env,
          args.logdir, start_iter + args.interval, args.add_label,
