...
latex_header += '''
\\multicolumn{6}{c}{Episode Return $\\uparrow$ }\\\\
\\midrule
'''
...
for m in methods:
  ...
  latex_header += "{}".format(
      method_name_mapping[m])
  for e in envs:
    file_name = os.path.join(path, "{}_return.json".format(e))
    with open(file_name) as f:
      data = json.load(f)
    ...
    latex_header += "& ${} \scriptstyle{{\pm {} }}$ ".format(
        np.round(float(episode_return_mean), 2),
        np.round(float(episode_return_std), 2)
    )
