...
class LSTMTRTExecutor(Executor):
  def __init__(
      self,
      realsense_device,
      robot_controller,
  ...
  def main_execution(self, execution_time):
    ...
    while True:
      ...
      # compute next action
      action, target_joint_pos, self.hidden_states = self.policy.get_action(
          robot_observation,
          self.hidden_states,
  ...
  def warmup_observations(self):
    ...
    self.hidden_states = np.zeros(
        self.policy.pf.h_num_layers * self.policy.pf.h_dim
    )
    for i in range(self.frame_extract * 3 + 1):
        # fill action
      action, target_joint_pos, self.hidden_states = self.policy.get_action(
          self.robot_controller.get_observation(),  # phase_info,
          self.hidden_states, self.curr_frame, self.depth_scale,
          self.last_action
      )  # force frame update
