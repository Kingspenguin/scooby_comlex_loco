    ...
    if self.is_testing:
      ...
      while (not self.log_video or (step < self.vid_log_step)) and not self.vec_env.task.task_wrapper.test_finished:
        with torch.no_grad():
          ...
          pose_traj.append(infos["pose"])
          pos_traj.append(infos["pos"])
      ...
      reward_sum = np.vstack(reward_sum)[:, 0]
      episode_length = np.vstack(episode_length)[:, 0]

      if self.vec_env.task.task_wrapper.testing:
        with open(os.path.join(
            self.vec_env.task.task_wrapper.statistics_logdir,
            self.vec_env.task.task_wrapper.test_id + '_return.json'), 'w'
        ) as f:
          json.dump({
              "episode_return": [
                  str(np.mean(reward_sum)), str(np.std(reward_sum))
              ],
              "episode_length": [
                  str(np.mean(episode_length)), str(np.std(episode_length))
              ],
          }, f)

      if self.log_traj:
        import pytorch3d
        for pose, pos in zip(pose_traj, pos_traj):
          # Isaac Real part last, pytorch3d real part first
          quat = torch.cat(pose[..., 3:], pose[..., :3])
          rotation_mat = pytorch3d.transforms.quaternion_to_matrix(quat)
      ...
      for it in range(self.current_learning_iteration, num_learning_iterations):
        ...
        for c in range(self.num_transitions_per_env):
          ...
          if self.eval_env_nums > 0:
            with torch.no_grad():
              actions, actions_log_prob, values, mu, sigma, eval_log_prob = self.actor_critic.act(
                  current_obs, current_states, return_eval_log_prob=True)

              actions[-self.eval_env_nums:] = mu[-self.eval_env_nums:]
              actions_log_prob[-self.eval_env_nums:] = eval_log_prob[-self.eval_env_nums:]
          else:
            with torch.no_grad():
          ...
          self.storage.add_transitions(
              ...
              values, actions_log_prob, mu, sigma
          )
          current_obs.copy_(next_obs)
