...
from .teacher_storage import RolloutStorage
...
class RMATeacher:
      ...
      for it in range(self.current_learning_iteration, num_learning_iterations):
        ...
        for _ in range(self.num_transitions_per_env):
          if self.apply_reset:
            ...
            current_states = self.vec_env.get_state()
          # Compute the action
          observations, states, env_factor, inner_state_size = self.split_obs(
            current_obs)
          actions, actions_log_prob, values, mu, sigma = self.actor_critic.act(
            observations, states, env_factor=env_factor, inner_state_size=inner_state_size)
          # Step the vec_environment
          next_obs, rews, dones, infos = self.vec_env.step(actions)
  ...
  def split_obs(self, obs_batch):
    ...
    states = obs_batch[:, :self.vec_env.task.state_obs_size -
    ...
    height = obs_batch[
      :,
      self.vec_env.task.state_obs_size: self.vec_env.task.state_obs_size +
      self.vec_env.task.height_map_obs_size
    ]
    env_factor = torch.cat([
      privilege_info, height
    ], dim=1)
    observations = obs_batch[:, :  self.vec_env.task.state_obs_size +
                             self.vec_env.task.height_map_obs_size]
    inner_state_size = self.vec_env.task.state_obs_size - \
      self.vec_env.task.privilege_info_len
    # print(height_obs.shape)
    # print(vision_obs.shape)
    return observations, states, env_factor, inner_state_size
  ...
  def update(self):
    ...
    for epoch in range(self.num_learning_epochs):
      ...
      for indices in batch:
        ...
        observations_batch, _, env_factor_batch, inner_state_size = self.split_obs(
          obs_batch)
        actions_log_prob_batch, entropy_batch, value_batch, mu_batch, sigma_batch = self.actor_critic.evaluate(observations_batch,
                                                                                                               states_batch,
                                                                                                               actions_batch, env_factor=env_factor_batch, inner_state_size=inner_state_size)
        # KL
