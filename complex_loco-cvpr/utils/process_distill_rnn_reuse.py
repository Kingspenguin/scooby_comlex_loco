...
from rl_pytorch.distill_rnn_reuse import DistillRNNLMReuseTrainer, ActorCriticRNNLM


def process_distill_rnn_reuse_trainer(args, env, cfg_train, teacher_logdir, student_logdir):
  ...
  teacher_ac_class = ActorCritic
  if cfg_train["policy"]["teacher_use_rnn"]:
    teacher_ac_class = None
  """Set up the PPO system for training or inferencing."""
  trainer = DistillRNNLMReuseTrainer(
      vec_env=env,
      teacher_actor_critic_class=teacher_ac_class,
      student_actor_critic_class=ActorCriticRNNLM,
      ...
      teacher_use_rnn=cfg_train["policy"]["teacher_use_rnn"],
